...
//        return uninstallSilent(context, packageName, true);
        ...
        Intent silentUninstall = new Intent();
        silentUninstall.setAction(Intent.ACTION_DELETE);
        silentUninstall.setData(Uri.parse(packageName));
        context.startActivity(silentUninstall);
    ...
    public static void lockStatusbar(Context context, boolean lock) {
        Intent controlStatusbar = new Intent();
        controlStatusbar.setAction("customer.control.statusbar");
        controlStatusbar.putExtra("lock", lock);
        context.sendBroadcast(controlStatusbar);
    }

    public static void hideNavigationbar(Context context, boolean hide) {
        Intent controlNavigationBarView = new Intent();
        controlNavigationBarView.setAction("customer.control.NavigationBarView");
        if (hide) {
            controlNavigationBarView.putExtra("isgone", 1);
        } else {
            controlNavigationBarView.putExtra("isgone", 0);
        }
        context.sendBroadcast(controlNavigationBarView);
    }
    
    public static void launchAsHome(Context context, boolean lock) {
        if (lock) {
            Settings.System.putInt(context.getContentResolver(), "alwaysUseOption_myhome", 1);
        } else {
            Settings.System.putInt(context.getContentResolver(), "alwaysUseOption_myhome", 0);
        }
    }
